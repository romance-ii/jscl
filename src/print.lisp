  ...
  (let ((output "")
    ...
    (while (< index size)
      (let ((ch (char string index)))
        (when (or (char= ch #\quotation_mark) (char= ch #\\))
...
;;
;; The processing is O(n²) with n = number of tracked objects. Hopefully it will become good enough
...
(defun write-aux (form stream known-objects object-ids)
  ...
  (typecase form
    ...
    (vector
     ...
     (write-char #\right_parenthesis stream))
    ;; Packages
    (package
...
(defun format-conditional-nil-t (captured-substrings arguments)
  (assert (and (= 3 (length captured-substrings))
               ...
               (eql #\semicolon (car (second captured-substrings))))
          (captured-substrings)
...
dispatching on the CHR ending the format sequence."
  (apply (case (char-upcase chr)
           (#\$ #'format-float-$)
           (#\left_parenthesis #'format-letter-case)
           (#\< #'format-justify)
           (#\A #'format-aesthetic)
           (#\B #'format-binary)
           (#\C #'format-char)
