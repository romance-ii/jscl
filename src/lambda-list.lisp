    ...
    (cons
     (let ((variable (first desc))
       ...
       (unless (symbolp supplied-p-parameter)
         (error "`~S' is not a valid supplied optional parameter."
...
(defun parse-keyvar (desc)
  (etypecase desc
    (symbol
     (make-keyvar :variable
                  desc :keyword-name (intern (string desc) "KEYWORD")))
    (cons
     (let (variable
           keyword-name
           (initform (second desc))
...
(defun parse-destructuring-lambda-list (lambda-list)
  (let (;; Destructure lambda list structure where we accumulate the
    ...
    (flet (;; Check if we are in the beginning of the section NAME in
           ...
           ;; looking for a lambda list  keyword in the current position
           ;; of the lambda list.
           (in-section-p ()
      ...
      (when (lambda-section '&whole)
        (setf (lambda-list-wholevar ll) (var-or-pattern
                                         (pop lambda-list))))

      (when (lambda-section '&environment)
        (setf (lambda-list-environment ll) (var-or-pattern
                                            (pop lambda-list))))
      
      ;; required vars
      (while (in-section-p)
        (push (var-or-pattern (pop lambda-list)) (lambda-list-reqvars ll)))
      ...
      ;; Dotted lambda-list and &rest/&body  vars. If the lambda-list is
      ;; dotted, convert the tail to a &rest and finish.
...
(defun nth-chain (x n &optional tail)
  "Return a chain  of the form (CAR  (CDR (CDR ... (CDR  X))), such that
there are N calls to CDR."
  (cond ((null tail)
         `(car ,(nth-chain x n t)))
        ((zerop n) x)
        (t `(cdr ,(nth-chain x (1- n) t)))))

(defvar *bindings*)
(defun compute-bindings (ll form)
    ...
    ;; LL, so we validate the number of elements on the result of FORM.
      ...
      ;; validation of keywords if there is any.
             ...
             (pattern (or restvar
              ...
              ;; If  there  is  keywords,   then  validate  this  list.
              ;; If there  is no keywords  and no &rest  variable, then
              ;; validate that the  rest is empty, it is  to say, there
              ;; is no more arguments that we expect.
                             (cond
...
(defun compute-pbindings (pattern form)
  "Compute  the bindings  for  a  pattern against  FORM.  If PATTERN  is
a lambda-list the  pattern is bound to an  auxiliary variable, otherwise
PATTERN must be a symbol, which will  be bound to the form. The variable
